...
- **Tests: 17 action tests + 3 canAction-gate tests added → 256/256 backend pass.** Each action happy-path (incl. linkedEntity correctness + assignee actually changed), RBAC denial (GL/Staff 403, other-division Manager 403, own-division Manager 200, any Manager on Org 200), RAISE_FINDING restrictions (non-eligible template → 400 + flag stays PENDING via rollback; non-task source → 400), DISSEMINATE reuses the same flag (count unchanged) + ORG card w/ taggedDivisionIds, final-state not re-actionable (400), dual-write (AuditLog + target-feed SYSTEM_EVENT), and validation (404/400/401). New fixtures: ESC-002 (allowsFindings:true) + ESC-003 (allowsFindings:**false**, since the schema default is `true`) templates and tasks; `afterAll` widened to clean `ESC-` templates + findings.
- **Post-Phase-4 code review (high-effort) — fixed in-line:** (1) **canAction gate** — `EscalationCard` showed action buttons to any Director/Admin/Manager regardless of division; now `getFeed` marks each ESCALATION_CARD with a server-computed `canAction` (same `canActionFlag`, resolving the WP division once per feed) and the card gates on it (+3 tests). (2) **EVENT_TYPES divergence** — the modal had a *different* event-type list than `RaiseFindingPanel`; both now import the canonical `constants/findingEventTypes.ts`. (3) **apiError helper** — the `{response:{data:{message}}}` cast (4 feed components) is now `utils/apiError.getApiErrorMessage`. (4) `resolveFlagDivision`'s dead `.find()`/fallback simplified to `cards[0]`.
- ➡️ **For Phase 5 (MUST clear before the feature is done):** gate the Header bell poll to Director/Admin/Manager (#22); add a flag-dedup guard (#21); add a dedicated escalations list page; extract the duplicated `formatTimestamp`/`sourceHref` helpers from `EscalationCard`/`InfoCard` (#23); refresh the Header/Sidebar badge counts after an action; update `CLAUDE_HANDOVER.md` + `BUSINESS_WORKFLOW.md` (Rule 12, after user confirms). **Carry-over code-review cleanups (low-risk, deferred — do NOT drop):** (a) `EscalationActionModal` re-fetches templates via `apiClient.get('/templates')` + a Published filter, duplicating `app/dashboard/tasks/new/page.tsx` — extract a shared `getPublishedTemplates()`; (b) `feed.controller.getFeed` runs `enrichAuthors` then `enrichFlagStatus` (and the WP-division lookup) sequentially — parallelise the two enrichment queries / fold them into one pass to shave a round-trip off every feed read.
