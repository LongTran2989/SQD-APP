...
*Last updated: 2026-06-29 (rev 21). Supersedes all previous versions.*
...
- **Deferred:** **DEF-8** (auth rate limiter is in-memory + IP-only; no per-account lockout — needs Redis + `employeeId` lockout), **DEF-9** (forgot-password timing enumeration), AR-8 (the inline `LOGOUT` audit write still lacks the best-effort guard), plus minor UX (AU-U6..U9) and AU-L2 (cookie flags keyed on `NODE_ENV`).
- **Tests:** backend **653/653** (was 649; +4 register-scope cases). Verified live against a Postgres + backend + Next.js (headless-Chromium) stack.
