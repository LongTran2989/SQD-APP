...
*Last updated: 2026-07-02 (rev 26). Supersedes all previous versions.*

> **rev 26 (2026-07-02):** **VPS demo nuke-and-rebuild fix: missing migration for `TemplateSet`/`WpBlueprint.externalRef`.** While running the documented "Nuke and rebuild for a demo" sequence (`TEST_P1_DEPLOYMENT_GUIDE.md`) against the VPS, `scripts/vps-ops.sh --reset-db --seed-mockup` failed both seed steps with `P2022: column '(not available)' does not exist`. Root cause: `974a7a1` added `externalRef` to both `TemplateSet` and `WpBlueprint` in `schema.prisma` without a matching migration file — every dev DB had already picked up the columns via an old `db push`, so it went unnoticed until a real `migrate reset --force` on an empty DB (the VPS). Added migration `20260702021416_add_external_ref_to_templateset_wpblueprint` (additive, nullable `TEXT` + unique index on both tables); verified with a real `migrate reset --force` + both seed scripts against `sqd_qa_test_db` (user-consented one-time destructive run per the AI-safety prompt Prisma raises for this command). Also fixed `task.test.ts` T51's hardcoded past-tense deadline (`2026-07-01` → `2099-07-01`), which started failing once the calendar passed that date. **No other schema changes. Backend 685/685.** See §8 gotcha #72.
>
> **rev 25 (2026-07-02):** **Assignee search typeahead rolled out to Work Package assignment and Task assign/reassign/transfer; division shown in every user picker.** No schema changes, no new endpoints. (1) **Division in user pickers:** the `users` datasource (`datasource.controller.ts::getDataSource`) now selects `division: { select: { code: true } }` and appends it to the label (`Nguyen Van A (VAE00123) · QA`) — a single shared change since every picker below reads this one endpoint; `datasource.test.ts` only asserts on `value`/ids, not label format, so it stayed green untouched. (2) **`TaskActionBar.tsx`:** the local `UserSelect` (plain `<select>` + a full, unfiltered `getUsers()` preload gated by a `needsUserList` effect) is now a thin wrapper around `AsyncSearchableSelect` calling `getDatasource('users', {q, limit: 20})` — upgrades all 4 flows that shared it (Assign unassigned task, Post-rejection reassign, General reassign, Transfer issuer rights) in one shot. Transfer-issuer's "exclude self" moved from filtering a preloaded array to a new `excludeUserId` prop that filters the async results. Removed the now-dead `allUsers` state, its preload `useEffect`, and the local `UserOption` type. (3) **`WorkPackageAssignmentPanel.tsx`:** same swap — the assign-modal's plain `<select>` + full-preload `getUsers()` → `AsyncSearchableSelect` + `getDatasource('users', {...})`; the "already assigned" exclusion moved to a post-fetch filter (`fetchAvailableUsers`) instead of filtering a preloaded array. Dropped the "All users are already assigned" message, which depended on knowing the full pool size upfront — the async picker's own "No results" messaging covers it now. **Backend `tsc --noEmit` clean, `datasource.test.ts` 6/6 passing; frontend `tsc --noEmit` clean on all touched files** (pre-existing lint debt — `any` catch clauses, gray-on-color findings on untouched lines in `WorkPackageAssignmentPanel.tsx` — confirmed via diff to predate this change, left as-is). Not verified in a live browser this session (no browser tool available) — spot-check the Task detail assign/reassign/transfer flows and the WP assignment modal before shipping.
>
> **rev 24 (2026-07-01):** **Home dashboard: Staff task visibility, drill-through links, quick-open feed, layout pass.** No schema changes, no new privilege keys. (1) **New `GET /dashboard/tasks`** (`dashboard.controller.ts::getTasks`) returns the signed-in user's active tasks (`assignedToUserId`, `status notIn ['Closed']` — same definition as the existing `myPendingTasks` summary count); new `MyTasksWidget` renders it on the Staff dashboard above Work Packages (reuses `TaskStatusBadge`). (2) **Drill-through links:** `DetailedMetricWidget` breakdown rows (and the promoted "Overdue" figure) now link to a pre-filtered list — Findings rows use the Findings page's existing `?status=` param; Task rows use `?status=`/`?dueFilter=today`/`?overdueOnly=true`/a **new `?pendingRatingOnly=true`** (added to `buildTaskFilters` in `task.controller.ts`: Closed + `rating: null`). The Tasks list page (`dashboard/tasks/page.tsx`) did not previously read the URL at all (sessionStorage-only) — it now seeds `statusFilters`/`dueFilter`/`overdueOnly`/`pendingRatingOnly` from `useSearchParams()` on load, wrapped in the `Suspense` boundary `useSearchParams` requires (mirrors the Findings page's existing pattern), and the sessionStorage hydration effect now no-ops whenever the URL carries any filter so a drill-through link is never immediately clobbered by a stale saved view. (3) **Activity feed quick-open:** TASK/WP/FINDING scope chips in `ActivityFeedWidget` now call `useQuickView().openTask/openWp/openFinding`, reusing the exact `text-blue-600 hover:underline` button affordance already established in `CapaPanel.tsx`/`PersonnelTab.tsx`/etc.; DIVISION/ORG chips stay plain text (no quick-view exists for those). (4) **Layout pass** (`/impeccable critique` + `layout`): top metrics rebuilt from 3 separate hero-metric cards into one unified divided stat-strip; Escalations now render as a full-width red banner above the strip when active (elevated priority) or fold into the strip as a calm cell when clear; "Overdue" promoted out of the 5-row Pending-Tasks breakdown into a distinguished top-line figure (fixes a ≤4-chunking violation); `QuickActionBar` is no longer `sticky` (was permanently eating viewport height stacked under the already-sticky `Header`). (5) **Investigated a "WP assigned but invisible on dashboard" report** (user VAE02682 / WP DEMO-WP-018) — **not a bug**: the WP's status is `Closed`, and both `getWorkPackages` and the widget itself (titled "Active Work Packages") intentionally exclude Closed/Inactive WPs. Checked all 65 `WorkPackageAssignment` rows in dev DB — 19 (Closed/Inactive) correctly excluded, 46 correctly visible, no orphaned/soft-delete mismatches. See §8 gotcha #70. **No new backend tests added for `getTasks`/`pendingRatingOnly`** — full suite still green (**backend 685/685**, unchanged count); frontend `tsc --noEmit` + lint clean on all touched files (pre-existing lint debt on untouched lines left as-is). Not verified in a live browser this session (no browser tool available) — spot-check before shipping.
>
> **rev 23 (2026-06-30):** **Google Sheet WP Sync `/code-review` (high effort)** on branch `claude/google-sheet-wp-sync-po3zss`. 9 findings (8 CONFIRMED, 1 PLAUSIBLE), all fixed. Headlines: (1) **B1** — `previewData` body in `executeSyncHandler` now validated by `PreviewDataSchema.safeParse` (exported from the service); (2) **B2** — `createOne`/`toUpdate` now enforce actor division scope (`actorDivisionIds` Set; Director/Admin bypass); (3) **C5** — `POST /execute` upgraded to `requireAnyPrivilege('wp:create', 'wp:edit')`; (4) **B4** — duplicate active WP names detected via `nameCounts`/`duplicateNames`; ambiguous rows pushed to a new `preflightErrors` array surfaced in preview; (5) **A5** — toUpdate triple-write now in `prisma.$transaction` (Rule 3 compliance); (6) **C6** — `result.created++` moved before `fireAutoGenForWp`, which is now try/catch-isolated; (7) **B3** — TAT `.nonnegative()` → `.positive()` (blank TAT=0 no longer silently routes to PC-EQ); (8) **A4** — missing collision decision key defaults to `'skip'` via `?? 'skip'`; (9) **B5** — concurrent-sync race documented in catch. +14 backend tests. **Backend 674/674.** See `CODE_REVIEW_AUDIT_LOG.md` 2026-06-30 session and §8 gotchas #45–47.

> **rev 22 (2026-06-29):** **Auth audit follow-ups** (same branch `claude/auth-audit-5dm9ju`, after rev 21's review). (1) **AR-8** — the inline `LOGOUT` AuditLog write now goes through the best-effort `writeAuthAudit`, so an audit failure can't 500 a logout that already revoked the session. (2) **AU-U8** — session-invalidation 401s (another tab / another device / disabled account) now surface a friendly amber notice on `/login` (via a `sessionStorage` reason set in `client.ts`) instead of a silent redirect. (3) **Multi-tab decision** — rather than weaken the cross-user `X-Acting-User-Id` guard, exposed **`ENFORCE_SINGLE_SESSION`** in the UI: a new **Admin/Director Security settings tab** + `GET`/`PUT /api/settings/security` (reads/writes the SystemSetting the auth middleware already consumes; default ON; `SECURITY_SETTING_CHANGED` audit). Gated by a **new `settings:security` privilege key** (default Director + Admin; configurable in the matrix) — **the first new privilege key since Phase 7** (catalog + `DEFAULT_PRIVILEGES`; no seed migration needed — `hasPrivilege` falls back to defaults per-key). **Backend 660/660.** Verified live (Director + Admin reach it; Staff 403; toggle persists; U8 notice shows). See `CODE_REVIEW_AUDIT_LOG.md` 2026-06-29 Part C and §8 gotcha #69.

> **rev 21 (2026-06-29):** **Authentication audit + remediation + `/code-review`** on branch `claude/auth-audit-5dm9ju` — a manual audit of the whole auth surface (controller, middleware, routes, rate limiter, env/CORS/cookies, and the frontend auth flow) and remediation of the accepted subset. Backend: **server-side password policy** (`utils/passwordPolicy.ts`, enforced on register/update/reset; reject reuse — AU-H1); **authentication audit logging** (`LOGIN_SUCCESS`/`LOGIN_FAILED`/`USER_REGISTERED`/`PASSWORD_CHANGED`/`PASSWORD_RESET[_REQUESTED]` via best-effort `writeAuthAudit`, AuditLog-only — AU-H2); **register role-elevation + division scope** hardened (only a Director may mint a Director **or** Admin; non-cross-division creators confined to their own division via the shared `hasCrossDivisionReach`; numeric `divisionId` coercion; division-existence check — AU-H3 / AR-1..4); **JWT `HS256` pinned** on sign+verify (AU-M1); forced-change users may reach `/logout` (AU-M4); email-format validation (AU-L1). Frontend: reusable **`PasswordInput`** (show/hide toggle, keyboard-reachable, `useId` label assoc) and **`PasswordStrength`** (mirrors the server policy incl. the 72-byte cap; shared `isPasswordValid` so client gating matches the server) wired into login/update/reset; 429 surfaced on login; stale forgot-password placeholder fixed. Then an accepted high-effort `/code-review` of the diff (AR-1..10) — the headline AR-1 was that the division-scope guard was **dead code with a false-positive test** (only Director/Admin hold `user:create` by default, both global), now actually exercised. **Backend 653/653** (was 649; +4 register-scope tests). Frontend `tsc`/ESLint clean on touched files; verified live against a Postgres + backend + Next.js (headless-Chromium) stack. **No schema migration, no new privilege keys.** Deferred: **DEF-8** (Redis-backed + per-account auth rate limiting), **DEF-9** (forgot-password timing enumeration), AR-8 (refactor the `LOGOUT` audit write to the best-effort helper). See `CODE_REVIEW_AUDIT_LOG.md` 2026-06-29 and §8 gotcha #68. Pre-existing frontend lint debt (~113 problems in untouched files) left for a dedicated cleanup pass.
...
### Completed
- **Assignee search typeahead rolled out (Task actions + Work Package assignment) + division in user pickers** (✅ **COMPLETE**, 2026-07-02)
  > The searchable/debounced `AsyncSearchableSelect` + `GET /api/datasources/users` pattern (already used in `TaskCreateForm.tsx`/`QuickTaskForm.tsx`) replaces the last plain `<select>` + full-unfiltered-preload user pickers: `TaskActionBar.tsx`'s shared `UserSelect` (Assign unassigned task, Post-rejection reassign, General reassign, Transfer issuer rights) and `WorkPackageAssignmentPanel.tsx`'s assign modal. The `users` datasource label now includes the division code (`Name (EmployeeId) · CODE`) for every caller. No schema changes, no new endpoints — reused the existing datasource. Backend `tsc` clean + `datasource.test.ts` 6/6; frontend `tsc`/lint clean on touched files. Not verified live in a browser this session. See rev 25 note above.
- **Home Dashboard: Staff task visibility, drill-through links, quick-open feed, layout pass** (✅ **COMPLETE**, 2026-07-01)
  > New `GET /dashboard/tasks` + `MyTasksWidget` gives Staff visibility into their active tasks alongside Work Packages on the home dashboard. `DetailedMetricWidget` breakdown rows now drill through to pre-filtered Tasks/Findings lists (new `pendingRatingOnly` task filter; Tasks page now URL-seeds its filters, previously sessionStorage-only). `ActivityFeedWidget` scope chips (Task/WP/Finding) are now quick-view triggers via the existing `useQuickView()` provider. Dashboard layout redesigned (unified stat-strip, escalation banner elevation, Overdue promoted out of a 5-row breakdown, `QuickActionBar` de-stickied) following an `/impeccable critique` + `layout` pass. A user-reported "WP assigned but missing from dashboard" turned out not to be a bug (Closed WP correctly excluded by the widget's own "Active Work Packages" scope — see §8 gotcha #70). No schema changes. **Backend 685/685** (no new tests added this session — `getTasks`/`pendingRatingOnly` are untested; consider backfilling). Frontend `tsc`/lint clean on touched files. Not verified live in a browser (no browser tool available this session). See rev 24 note above.
...
45. **`PreviewData.preflightErrors` is required on the backend but optional (`?`) on the frontend (`sheetSyncTypes.ts`):** The distinction is intentional — the backend always populates the field (as `[]` when empty), but the frontend type uses `preflightErrors?` to avoid TypeScript errors on pre-existing helpers that construct `PreviewData` inline (e.g. `emptyPreview()` in tests). If you add a new call site on the frontend that explicitly constructs a `PreviewData` object, include `preflightErrors: []`.

46. **`tx as unknown as typeof prisma` cast in `googleSheetSync.service.ts` `executeSync`:** Prisma's interactive-transaction client (`Prisma.TransactionClient`) is structurally incompatible with `typeof prisma` because it omits top-level transaction/`$on` methods. The double-cast allows passing `tx` to `createFeedPost`, which is typed `(prisma: PrismaClient) => ...` but only calls model methods internally. Use this pattern only when you control the callee and are sure it does not call `.$transaction`, `.$on`, or similar top-level client methods inside the transaction.

47. **Sheet sync concurrent-`-REV` race (B5) is documented but not eliminated:** `findAvailableName` scans for an available `-REV2`…`-REV9` slot and `createOne` creates; the two steps are not atomic. A concurrent sync run can claim the same slot between the scan and the write, producing a Prisma unique-constraint violation. The catch block surfaces this as a user-visible `errors[]` entry ("Name taken by concurrent sync — retry") rather than a 500. The window is narrow (two workers would need to resolve the same `wpNo` name in the same millisecond), but if concurrent syncs become a real concern, wrap both steps in a `SELECT ... FOR UPDATE` advisory lock or use a DB sequence for the revision counter.
...
69. **Security settings + the `settings:security` privilege (2026-06-29):** `ENFORCE_SINGLE_SESSION` is a string `SystemSetting` the auth middleware reads on every request (default ON when the row is missing/`'true'`). It's now Admin/Director-configurable via `GET`/`PUT /api/settings/security` (controller `securitySettings.controller.ts`) and the **Security** settings tab. The endpoint is gated by a **new privilege key `settings:security`** — the first added since Phase 7. Adding a privilege key means: extend the `PrivilegeKey` union, `PRIVILEGE_CATALOG` (drives the matrix UI), and `DEFAULT_PRIVILEGES` (granted to Director + Admin here). **No seed migration is needed** — `seedPrivileges` uses `update: {}` (never clobbers existing configs) and `hasPrivilege` falls back to `DEFAULT_PRIVILEGES` per-key, so pre-existing seeded configs that predate the key still resolve correctly. Frontend tab visibility is role-gated (`isAdminDirector`) while the backend enforces the privilege — same split as the other settings tabs (an Admin who revokes the key in the matrix will see a 403, not a hidden tab). The toggle write logs `SECURITY_SETTING_CHANGED`. The cross-user multi-tab guard (`X-Acting-User-Id`) is a SEPARATE, always-on security control — do NOT add a toggle to disable it (it only blocks a *different* user hijacking a shared-cookie tab; same-user multi-tab already works).

68. **Auth hardening invariants (2026-06-29, branch `claude/auth-audit-5dm9ju`):** (a) **Password policy is server-side and single-source** — `utils/passwordPolicy.ts` `validatePassword` (min 8, ≤72 bytes, letters+numbers) gates register / update-password / reset-password; the frontend `PasswordStrength.isPasswordValid` mirrors it. If you change the policy, change BOTH (they're deliberately kept in sync; the frontend file says so). (b) **Auth events write `AuditLog` only**, not `TaskActivity` — they have no Task context, so this is a *documented* exception to Rule 3 (dual-write), via the best-effort `writeAuthAudit` helper. Use `(details as Prisma.InputJsonValue) ?? Prisma.DbNull` for the JSON `details` column — the project's `exactOptionalPropertyTypes` rejects `details: undefined`. (c) **`register` scope:** only a Director may create a Director **or** Admin (`ELEVATED_ROLE_NAMES`); non-cross-division creators are confined to their own division via the shared **`hasCrossDivisionReach(req.user)`** (don't re-hand-roll the role-string check); `divisionId` MUST be coerced (`Number()`) before the scope compare and the Prisma `Int` lookup. The division-scope branch is only *reachable* if a non-global role is granted `user:create` via `PrivilegeConfig` (Director/Admin both bypass it as global) — the test grants Manager `user:create` so the guard is actually exercised (don't "simplify" that test or the guard silently goes untested again). (d) **JWT is `HS256`-pinned** on sign+verify — keep them in lockstep. (e) Forced-password-change users may reach `/logout` (in `FORCED_CHANGE_ALLOWED_PATHS`). See `CODE_REVIEW_AUDIT_LOG.md` 2026-06-29 and §11.
...
71. **User pickers standardized on `AsyncSearchableSelect` + `GET /api/datasources/users` — don't reintroduce a plain `<select>` + full preload (2026-07-02):** `TaskCreateForm.tsx`/`QuickTaskForm.tsx`/`TaskActionBar.tsx`/`WorkPackageAssignmentPanel.tsx` all now search server-side via `getDatasource('users', {q, limit, divisionId?})` (`datasource.controller.ts::getDataSource`, case `'users'`), which excludes Admin/Senior Advisor, dedupes on `deletedAt: null`, and formats the label as `Name (EmployeeId) · DivisionCode`. If you add a new user-picking UI, use `AsyncSearchableSelect` with this same `fetchOptions` shape rather than preloading the whole user table client-side (the old pattern doesn't scale and doesn't show division). To exclude specific ids (self, already-assigned, etc.), filter the results *inside* the `fetchOptions` closure (post-fetch) — filtering a preloaded array client-side no longer applies since there is no full array anymore.

72. **A `schema.prisma` field added without a matching migration only breaks on a genuinely fresh DB — it will look fine everywhere else (2026-07-02):** Commit `974a7a1` ("Add idempotent Excel seeding for Template Sets and WP Blueprints") added `TemplateSet.externalRef` and `WpBlueprint.externalRef` directly to `schema.prisma` but never generated a migration for them. Every local/dev database quietly picked up the columns via an earlier `npx prisma db push`, so `npx prisma migrate status` reported "up to date" and nothing looked wrong for over a week. The gap only surfaced on the VPS demo-reset flow (`scripts/vps-ops.sh --reset-db`, i.e. `prisma migrate reset --force` against a genuinely empty DB) — `migrate reset` replays only committed migration files, so the columns never got created there, while the generated Prisma Client (built from `schema.prisma`) still expected them, throwing `P2022: The column '(not available)' does not exist` from `seed-blueprints.ts` and `seed-mass-mockup-v2.ts`. Fixed with migration `20260702021416_add_external_ref_to_templateset_wpblueprint` (additive: two nullable `TEXT` columns + unique indexes), applied to `sqd_qa_db` via `prisma migrate resolve --applied` (already had the columns from the old `db push`) and verified end-to-end with a real `migrate reset --force` against `sqd_qa_test_db`. **Takeaway: `migrate status`/`db push` succeeding locally does not prove a migration file exists — before trusting a schema change is deployable, diff it against `git log -p -- backend/prisma/migrations` or just try a real `migrate reset --force` against an empty database.** Also fixed in the same pass: `task.test.ts` T51 had a hardcoded absolute deadline (`2026-07-01`) which started failing once that date passed, since past deadlines are rejected by design (see gotchas around SR-5/SR-6) — bumped to `2099-07-01`. **Backend 685/685.**
70. **A Closed/Inactive WP disappearing from a user's dashboard is expected, not a bug (2026-07-01):** `getWorkPackages` (dashboard) filters `status: { notIn: ['Closed', 'Inactive'] }` for everyone, and the widget is explicitly titled "Active Work Packages." `WorkPackageAssignment` rows are never cleaned up when a WP closes (they're the audit trail of who worked it), so it's normal for a meaningful fraction of a user's assignment history (checked: 19/65 = ~29% in dev data) to point at WPs that no longer show on the dashboard. Before treating "assigned but invisible" as a bug, check the WP's `status` first. Related: the Tasks list page's URL-seeded filters (gotcha above rev 24 note) — `useSearchParams().toString()` non-empty short-circuits the sessionStorage-hydration `useEffect` entirely, so a drill-through link always wins over a previously saved filter view; if you add a new persisted filter field to that page, seed its initial `useState` from the URL too or a dashboard link for that field will silently do nothing.
...
### 2026-06-29 audit — additional hardening (rev 21, branch `claude/auth-audit-5dm9ju`) — ✅ DONE
A second full audit of the auth surface + an accepted `/code-review`. **No schema change, no new privilege keys.** Full detail (every finding + status) in `CODE_REVIEW_AUDIT_LOG.md` 2026-06-29.
- **Password policy (AU-H1):** `utils/passwordPolicy.ts` (`validatePassword`: min 8, ≤72 bytes, letters+numbers) enforced server-side on register / update-password / reset-password; `updatePassword` also rejects `newPassword === oldPassword`. The frontend `PasswordStrength.isPasswordValid` mirrors it (single visible rule set incl. the 72-byte cap) so client gating can't promise what the server rejects.
- **Auth audit logging (AU-H2):** best-effort `writeAuthAudit` writes `LOGIN_SUCCESS`, `LOGIN_FAILED` (known accounts only — keeps the unknown-user path enumeration-neutral), `USER_REGISTERED`, `PASSWORD_CHANGED`, `PASSWORD_RESET_REQUESTED`, `PASSWORD_RESET`. AuditLog-only — auth events have no Task context (documented Rule-3 exception; mirrors the existing LOGOUT precedent). Uses `(details as Prisma.InputJsonValue) ?? Prisma.DbNull` (the project's `exactOptionalPropertyTypes` requires this — see gotcha #68).
- **Register scope (AU-H3 / AR-1..4, AR-9):** only a Director may create a Director **or** Admin (`ELEVATED_ROLE_NAMES`); creators without cross-division reach (`hasCrossDivisionReach`, shared with task/WP guards) are confined to their own division; `divisionId` is coerced to a positive integer (a string previously 403'd / 500'd); the target division must exist; the actor is read from the typed `req.user`. The division-scope branch is currently only reachable if a non-global role is granted `user:create` via `PrivilegeConfig` (defense-in-depth) — the test grants it so the guard is genuinely exercised.
- **JWT algorithm pinned (AU-M1):** `HS256` on both `jwt.sign` and `jwt.verify`.
- **Forced-change logout (AU-M4):** `/logout` is in `FORCED_CHANGE_ALLOWED_PATHS` so a forced user can end their session.
- **Frontend (AU-U1/U2/U4/U5, AR-5/6/7):** `PasswordInput` (show/hide toggle, keyboard-reachable, `useId`-backed label association) + `PasswordStrength` (live checklist + meter that never exceeds "Weak" until all rules pass) on login/update/reset; login surfaces 429; forgot-password placeholder de-staled.
- **Follow-ups (rev 22, same branch):** **AR-8** (logout audit write now best-effort) and **AU-U8** (friendly sign-out notice on `/login`) are now DONE; added the **`ENFORCE_SINGLE_SESSION` Security settings tab + `settings:security` privilege** (see gotcha #69 and §2 rev 22).
- **Deferred:** **DEF-8** (auth rate limiter is in-memory + IP-only; no per-account lockout — needs Redis + `employeeId` lockout), **DEF-9** (forgot-password timing enumeration), plus minor UX (AU-U6/U7/U9 — forced-change logout UX, server-side route gating, success-page dead-ends), AU-M3, and AU-L2 (cookie flags keyed on `NODE_ENV`).
- **Tests:** backend **660/660** (was 649; +4 register-scope, +7 security-settings cases). Verified live against a Postgres + backend + Next.js (headless-Chromium) stack.
...
> Lower-severity deferred items (DEF-1…DEF-9: DOMPurify-on-import, `SearchableSelect` keyboard a11y,
> issuer-transfer division scope, `task:assign_div` target-division check, `FILE_UPLOAD_CONFIG` PUT
> endpoint, attachment per-entity scope, task `skillLevel` not enforced vs assignee [DEF-7], auth rate
> limiter is in-memory + IP-only with no per-account lockout [DEF-8], forgot-password timing enumeration
> [DEF-9]) remain tracked in `CODE_REVIEW_AUDIT_LOG.md` — none are prod blockers at the current
> privilege matrix.

### 12.9 Analytics Adjustments (2026-06-30)

- **Efficiency Formula Inverted**: `actual / estimated` was replaced by `estimated / actual`. Values ≥ 1.0 indicate on-time or under-budget (efficient). Efficiency badge logic was updated so that ≥ 1.0 is green and < 1.0 is red.
- **On-Time Rate**: Replaced `Rejection Rate` in the Personnel Performance Analytics. It calculates the percentage of closed tasks completed on or before the deadline.
- **Tasks Completed**: Added to the Personnel table as a direct count of final-state tasks used in the performance analytics calculation.
