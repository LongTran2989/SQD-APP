...
- **One feed, five scopes:** All discussion and system events live in a single `FeedPost` model across five scopes — **Task**, **Work Package**, **Division Board**, the org-wide **Org Feed**, and **Finding** (the finding activity feed). The per-task activity feed is just the Task scope of this feed.
...
- **Disseminate tags are informational only:** When disseminating to the Org Feed, the optional "tagged divisions" are validated (they must reference real divisions; unknown/duplicate ids are dropped) but they do **not** scope who can see the card — the Org Feed is org-wide readable by design (intentional transparency). Tags are a labelling/awareness aid, not access control.

### Feed collaboration features (2026-06 — Phases A–H)
- **Mentions:** A comment can **@mention** colleagues (picked from a search box, shown as chips). Mentioned users get a notification. Mentioning is open to anyone (it only notifies — it does not grant or restrict access).
- **Entity links:** Typing a business code with a leading **`#`** (e.g. `#FED-000001`, `#FND-000123`, a WP code) turns it into a clickable link to that Task / Work Package / Finding **if the code is real**; unknown codes stay plain text. Nothing is "shared" — it's a convenience link within the existing transparency model.
- **Attachments:** A comment can carry **file attachments** (evidence, photos, PDFs). The same upload policy, size/type limits, and per-record quota as task/finding attachments apply; files are soft-deleted and downloaded through the app, never a public link.
- **Pinning:** On the **Work Package, Division Board, and Org** feeds, an authorised poster (Director/Admin anywhere; Manager on the Org Feed; division members on their own board) can **pin** a comment so it stays in a "Pinned" strip at the top — for standing announcements/directives. Tasks and Findings are not pinnable.
- **Hiding (moderation):** A **Director/Admin** can **hide** an inappropriate comment. Hidden comments disappear from every feed for everyone; they are **not deleted** (the record and audit trail are kept), and a Director/Admin can reveal hidden comments to review or restore them. Hiding and pinning are both recorded in the audit trail + a feed system event.
- **Acknowledgement:** Any user can mark a comment **"Acknowledged"** ("I have read this") — useful for directives. The comment shows how many people acknowledged it; each person can acknowledge once. Hidden comments cannot be acknowledged.
- **Search:** Each feed has a **search box** that finds comments by their text (hidden comments excluded).
- **Daily digest (opt-in):** A user can opt in (Preferences) to a once-daily summary notification of new **Org Feed + their Division Board** comment activity, for catching up after time away.
- **Large feeds load incrementally:** Feeds show the most recent activity first and load older entries on demand ("Load earlier") rather than the entire history at once.
