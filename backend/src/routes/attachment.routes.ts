...
// Files buffer in memory then stream to the storage backend. The size limit
// here is the fixed infrastructure memory-safety ceiling (NOT the configurable
// policy limit — that is enforced per-category in attachmentService). A single
// `file` part is accepted per request.
const upload = multer({
